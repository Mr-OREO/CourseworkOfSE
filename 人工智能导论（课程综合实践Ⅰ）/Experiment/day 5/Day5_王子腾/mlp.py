...
def MLP(input_text,keep_prob):
    ...
    with tf.name_scope('emd'):
        ...
        # 改变embedding形状 batch x len x emd-> batch x (len*emd)  三维变二维
        #tf.reshape
    ...
    with tf.name_scope("MLP"):
        ...
        fc1 = tf.layers.dense(embedding_input,config.embedding_dim)
        # 利用函数tf.nn.relu
        ...
        # 第二层同理如上
        fc2 =tf.layers.dense(fc1,config.embedding_dim/2)
        fc2 =tf.nn.relu(fc2)
