...
def CNN(input_text,keep_prob):
    ...
    with tf.name_scope("CNN"):
        ...
        conv1 = tf.layers.conv1d(embedding_input,config.num_filters,config.kernel_size)
        ...
        conv2 = tf.layers.conv1d(pool1,1,1)
    ...
    with tf.name_scope("MLP_output"):
        ...
        # 利用函数tf.layers.dense
        fc1 = tf.layers.dense(conv2,config.embedding_dim)
        # 利用函数tf.nn.relu
        fc1 = tf.nn.relu(fc1)
        # 利用函数tf.nn.dropout
        ...
        # 利用函数tf.argmax
        pred_label = tf.arg_max(norm_logits,1)
